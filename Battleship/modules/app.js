    ...
    if (!btn.disabled) {
      ...
      if (marekdMinesText >= 0) {
        if (btn.childElementCount !== 0) {
          if (!btn.classList.contains("fa-flag")) {
            ...
            btn.classList.add("fa-flag");
            btn.innerHTML = `<img src="./pic/flag.png" />`;
            markedMinesCounterUpdater(markedMinesCounter);
          } else if (btn.classList.contains("fa-flag")) {
            console.log("HEyooo, ", btn);
            btn.classList.remove("clicked");
            btn.firstChild.remove();
            btn.classList.remove("fa-flag");
            if (btn.classList.contains("minedOne")) {
              btn.innerHTML = `<img class="mine" src="./pic/mine.png"/>`;
            } else if (btn.value !== "") {
...
function addingMinesAndNumbers() {
  ...
  minesRandomResult.forEach((num) => {
    buttons[num].innerHTML = `<img class="mine" src="./pic/mine.png"/>`;
  });
  addNumbersIntoGame();
}
...
function isGameOver(btn) {
  ...
  if (btn.firstElementChild !== null) {
    if (
      ...
      btn.firstChild !== 0 &&
      !btn.classList.contains("fa-flag")
    ) {
      confirm(
